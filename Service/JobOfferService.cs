...
namespace vabalas_api.Service.Impl
{
    public class JobOfferService : IJobOfferService
    {
        ...
        public async Task<JobOffer> GetById(Guid offerId)
        {
        ...
        public async Task<List<JobOffer>> GetAllByUserId(string userId)
        {
            var user = await _userService.GetById(userId);
            var jobOfferList = await _context.JobOffers.Where(jo => (jo.Job.Owner == user)).ToListAsync();
        ...
        public async Task<IEnumerable<JobOffer>> GetAllByStatus(string userId, string status)
        {
            var user = await _userService.GetById(userId);
            
            return await _context.JobOffers
                .Where(jo => (jo.Job.Owner == user) && (jo.OfferStatus == OfferStatusParser.ToEnum(status)))
                .ToListAsync();
        }
